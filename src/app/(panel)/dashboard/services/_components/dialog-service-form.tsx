...
export function DialogServiceForm({ service, employeeId, onClose }: DialogServiceFormProps) {
    ...
    return (
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            ...
            {/* employeeId passado como hidden — FREE/BASIC manda vazio, PROFESSIONAL manda o id */}
            ...
            <div className="flex flex-col gap-1.5">
                ...
                <div className="flex gap-4">
                    <div className="flex flex-col gap-1 flex-1">
                        ...
                        <input name="hours" type="number" min="0" max="23" defaultValue={defaultHours} required className={inputClass} />
                    </div>
                    <div className="flex flex-col gap-1 flex-1">
                        <span className="text-[var(--on-surface-dim)] text-xs">Minutos</span>
                        <input name="minutes" type="number" min="0" max="59" defaultValue={defaultMinutes} required className={inputClass} />
                    </div>
                </div>
            </div>
            ...
            {service && (
                <div className="flex flex-col gap-1.5">
                    <label className="text-[var(--gold)] text-xs tracking-widest uppercase">Status</label>
                    <select
                        name="status"
            ...
            <div className="flex flex-col gap-2 mt-1">
                <button type="submit" disabled={loading} className="btn-primary w-full py-3.5 disabled:opacity-50 cursor-pointer">
                    {loading ? "Salvando..." : service ? "Salvar serviço" : "Cadastrar serviço"}
                </button>
                <button type="button" onClick={onClose} className="w-full border border-[var(--outline-variant)] text-[var(--on-surface-dim)] py-3 text-xs tracking-widest uppercase hover:text-[var(--on-surface-variant)] hover:border-[var(--outline)] transition-colors cursor-pointer">
                    Cancelar
                </button>
            </div>
        </form>
